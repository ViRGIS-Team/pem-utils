...
namespace PemUtils
{
    public class PemReader : IDisposable
    {
        ...
        public RSAParameters ReadRsaKey()
        {
            ...
            if (headerFormat.Equals(PemFormat.RsaPublic)) return ReadRsaPubliceKey(der);
        ...
        private static RSAParameters ReadRsaPubliceKey(DerAsnType der)
        {
            if (der == null) throw new ArgumentNullException(nameof(der));
            var sequence = der as DerAsnSequence;
            if (sequence == null) throw new ArgumentException($"{nameof(der)} is not a sequence");
            if (sequence.Value.Length != 2) throw new InvalidOperationException("Sequence must contain 9 parts");

            return new RSAParameters
            {
                Modulus = GetIntegerData(sequence.Value[0]),
                Exponent = GetIntegerData(sequence.Value[1])
            };
        }
